...
#   v0.90 - Create a Scheduler Version (run every 5 min)              #
#           1. Check for Achoir Collection                            #
...
def main():
    ...
    if os.path.isfile(".\plugins\compname.pl"):
        ...
        cmdexec = "Copy " + achname + "\\RRV\\RegRipper2.8-master\\plugins\\compname.pl .\\plugins\compname.pl"
        returned_value = os.system(cmdexec)
        cmdexec = "Copy " + achname + "\\RRV\\RegRipper2.8-master\\plugins\\shellfolders.pl .\\plugins\shellfolders.pl"
        returned_value = os.system(cmdexec)
        cmdexec = "Copy " + achname + "\\RRV\\RegRipper2.8-master\\plugins\\userassist.pl .\\plugins\\userassist.pl"
        returned_value = os.system(cmdexec)
        cmdexec = "Copy " + achname + "\\RRV\\RegRipper2.8-master\\plugins\\winnt_cv.pl .\\plugins\winnt_cv.pl"
        returned_value = os.system(cmdexec)
        cmdexec = "Copy " + achname + "\\RRV\\RegRipper2.8-master\\plugins\\winver.pl .\\plugins\winver.pl"
        returned_value = os.system(cmdexec)
    ...
    for roox, dirx, filex in os.walk(srcname):
        for dname in dirx:
            ...
            cmdexec = achname + "\\RRV\\RegRipper2.8-master\\rip.exe -p winnt_cv -r " + srcfull + "\Reg\SOFTWARE > SysInfo.dat"
            returned_value = os.system(cmdexec)

            cmdexec = achname + "\\RRV\\RegRipper2.8-master\\rip.exe -p compname -r " + srcfull + "\Reg\SYSTEM >> SysInfo.dat"
            returned_value = os.system(cmdexec)

            cmdexec = achname + "\\RRV\\RegRipper2.8-master\\rip.exe -p winver -r " + srcfull + "\Reg\SOFTWARE >> SysInfo.dat"
            returned_value = os.system(cmdexec)


            print("[+] Generating Prefetch Data...")
            cmdexec = achname + "\\SYS\\WinPrefetchView.exe /folder " + srcfull + "\prf /scomma WinPrefetchview.csv"
            returned_value = os.system(cmdexec)
            ...
            for root, dirs, files in os.walk(curdir):
                for fname in files:
                    ...
                    if fname.startswith("NTUSER."):
                        ...
                        cmdexec = achname + "\\RRV\\RegRipper2.8-master\\rip.exe -p shellfolders -r " + curfile + " > " + curouput
                        returned_value = os.system(cmdexec)

                        cmdexec = achname + "\\RRV\\RegRipper2.8-master\\rip.exe -p userassist -r " + curfile + " >> " + curouput
                        returned_value = os.system(cmdexec)

                        reccount = reccount + 1

            print("[+] Generating RDP Success and Failure...")
            cmdexec = "copy " + srcfull + "\\evt\\sys32\\Security.evtx"
            returned_value = os.system(cmdexec)

            print("[+] Generating Service Installed (7045) Messages...")
            cmdexec = "copy " + srcfull + "\\evt\\sys32\\System.evtx"
            returned_value = os.system(cmdexec)
            ...
            cmdexec = achname + "\\SYS\\RBCmd.exe -d " + srcfull + "\\RBin >> RBin.dat" 
            returned_value = os.system(cmdexec)
            ...
            ###########################################################################
            print("[+] Parsing $MFT...")
            cmdexec = achname + "\\DSK\\MFTDump.exe /l /d /v --output=MFTDump.csv " + srcfull + "\\RawData\\$MFT" 
            returned_value = os.system(cmdexec)
            ...
            # Fell Through, Now Process the files and extract data for report         #
            ###########################################################################
            print("[+] Now Processing AChoir Extraction: " + srcfull)
            ...
            outfile.write("</style><title>AChoir Endpoint Report(" + dname + ")</title></head>\n")
            outfile.write("<body>\n")
            outfile.write("<p><Center>\n")
            outfile.write("<a name=Top></a>\n<H1>AChoir Endpoint Report</H1>\n")
            outfile.write("(" + dname + ")<br>\n")
            ...
            reccount = 0
            filname = srcfull + "\\brw\\BrowseHist.csv"
            outfile.write("<table border=1 cellpadding=5 width=100%>\n")
            ...
            reccount = 0
            filname = srcfull + "\\sys\\CPorts.csv"
            ...
            reccount = 0
            filname = srcfull + "\\sys\\UserAssist.csv"
            ...
            writeRow = 0
            LastRec = ""
            filname = srcfull + "\\sys\\IPCfgDNS.dat"
